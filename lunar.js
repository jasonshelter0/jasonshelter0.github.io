...
function Lunar() {
  ...
  this.yueLiHTML = function (By, Bm, curJD) {
    ...
    var sty_ncDate_incomplete = ' style="font-family: Arial Black; text-align: center;font-size: 12px; color: ';
...
function std_nianLi2HTML(y) { //html年历生成
  var i, j, s = '', s1, s2, v, v2, qi;
  SSQ.calcY(int2((y - 2000) * 365.2422 + 180));
  for (i = 0; i < 14; i++) {
    if (SSQ.HS[i + 1] > SSQ.ZQ[24]) break; //已包含下一年的冬至
    if (SSQ.leap && i == SSQ.leap) s1 = '闰'; else s1 = '·';
    s1 += SSQ.ym[i]; if (s1.length < 3) s1 += '月'; // 月份
    s1 += SSQ.dx[i] > 29 ? '大' : '小';
    v = SSQ.HS[i] + J2000;
    s1 += ' ' // + obb.Gan[(v + 9) % 10] + obb.Zhi[(v + 1) % 12];
    s1 += ' ' + JD.JD2str(v).substr(6, 5);
...
function gCheckPoint(y, tgt) { //gCP点生成
...
function custom_write_nianli(year, target) {
  document.write(gCheckPoint(year, target));
}
function std_write_nianli(year) {
  document.write(std_nianLi2HTML(year));
}
function write_checkpoint(year, target) {
  document.write(checkpoint(year, target));
}
function write_testing(tgt) {
  var m, cri = 0;
  if (tgt >= 10) cri = 1;
  m = (tgt >= 10) * 7 + 9;
  document.write(m);
}
function write_and_read(s) {
  var gy, gm, gd;
  s = String(s).replace(/[^0-9:\.]/g, '');
  gy = s.split('.')[0];
  gm = s.split('.')[1];
  gd = s.split('.')[2];
  document.write(gy + ' : ' + gm + ' : ' + gd + ' <br>');
  document.write(typeof (gy) + '<br>');
  document.write(gy.toString().padStart(2, '0') + '.' + gm.toString().padStart(2, '0') + '.' + gd.toString().padStart(2, '0'))
  return gy.toString().padStart(2, '0') + '.' + gm.toString().padStart(2, '0') + '.' + gd.toString().padStart(2, '0');
}
function write_JN_and_NJ_and_SDRB(y, m, d) {
  // document.write(m);
  document.write(y.toString().padStart(2, '0') + '.' + m.toString().padStart(2, '0') + '.' + d.toString().padStart(2, '0') + '____');
  var arrN = JD.JN(Number(y), Number(m), Number(d));
  document.write(arrN[0].toString().padStart(2, '0') + '-' + arrN[1].toString().padStart(2, '0') + '-' + arrN[2].toString().padStart(2, '0') + '____');
  var arrJ = JD.NJ(Number(arrN[0]), Number(arrN[1]), Number(arrN[2]));
...
function write_everything(y, m, d) {
  ...
  if (arrJ[0] == y && arrJ[1] == m && arrJ[2] == d) {
    ...
    document.write(arrN[0].toString().padStart(2, '0') + '-' + arrN[1].toString().padStart(2, '0') + '-' + arrN[2].toString().padStart(2, '0') + '____');
    // document.write(arrJ[0].toString().padStart(2, '0') + '.' + arrJ[1].toString().padStart(2, '0') + '.' + arrJ[2].toString().padStart(2, '0') + '____');
    document.write(JD.JS(y, m, d).toString().padStart(5, '0') + '_' + JD.JgetWeekName(y, m, d).toString() + '____');
    // 2024.2.25, 
    document.write(JD.JgetSCOSSLOPSeq(y, m, d) + '_');
    document.write(JD.JgetSemWeekSeq(y, m, d, 2024, 2, 25, 20, '大三下') + "_");
    lun.yueLiCalc(y, m);
    var ob = lun.lun[d - 1];
    document.write(lun.Ly + lun.ShX + '年' + ob.Lleap + ob.Lmc + '月' + ob.Ldc + '__');
    var c = "";
    if (ob.A) c += '<font color=red>' + lun.substr2(ob.A, 6, '..') + '</font>';
    if (!c && ob.B) c = '<font color=blue>' + lun.substr2(ob.B, 6, '..') + '</font>';
    document.write(c + '__');
    document.write("<br>");
  } else {
